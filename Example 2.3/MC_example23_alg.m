...
% which uses the method in Breda et al, EJQTDE, 2016, i.e., by inverting the algebraic condition.
...
handles=feval(@PS_example23_alg);
opt=contset;
global cds;
...
[x0,v0]=init_EP_EP(@PS_example23_alg,state_eq,par0,ap1);
...
[x0,v0]=init_BP_EP(@PS_example23_alg,BP,parBP,sBP,0.001);
...
for index_sol = 1:size(xe,2)
    FM = @(x) 0.5*exp(xe(end,index_sol))*QuadWeights*f(polint(Nodes,[x;xe(1:end-1,index_sol)],QuadNodes)); 
    KM = fsolve(@(x) x-FM(x),xe(1,index_sol),opt_fsolve);
    b0(index_sol) = KM;
end
...
[x0,v0]=init_H_LC(@PS_example23_alg,H,parH,ap1,1e-3,ntst,ncol);
...
for ind_persol=1:size(xlc,2)
    for ind_mesh=1:ntst*ncol+1
        FM = @(x) 0.5*exp(xlc(end,index_sol))*QuadWeights*f(polint(Nodes,[x;xlc((ind_mesh-1)*MM+1:(ind_mesh-1)*MM+MM,ind_persol)],QuadNodes)); 
        KM = fsolve(@(x) x-FM(x),xlc(1,index_sol),opt_fsolve);
        b0_per = KM;
...
[xpd0,vpd0]=init_PD_LC(@PS_example23_alg,xlc,slc(SPD),ntst,ncol,1);
...
for ind_persol=1:size(xlc1,2)
    for ind_mesh=1:ntst*ncol+1
        FM = @(x) 0.5*exp(xlc1(end,index_sol))*QuadWeights*f(polint(Nodes,[x;xlc1((ind_mesh-1)*MM+1:(ind_mesh-1)*MM+MM,ind_persol)],QuadNodes)); 
        KM = fsolve(@(x) x-FM(x),xlc1(1,index_sol),opt_fsolve);
        b0_per = KM;
...
[x0,v0]=init_H_H(@PS_example23_alg,H,parH,[ap1 ap2]);
